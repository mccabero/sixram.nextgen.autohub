...
import { getProductGroups, deleteProductGroup } from '../../services/configService'
...
export default function ProductGroupsTable({ items }: { items?: Item[] }){
  ...
  useEffect(()=>{
    let mounted = true
    setLoading(true)
    getProductGroups().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[])

  function isProductGroupComplete(row: Item) {
    return hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isProductGroupComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isProductGroupComplete), [rows])
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/product-groups/add') }
  function handleEdit(id:number){ navigate(`/configuration/product-groups/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteProductGroup(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Organizational groups for inventory items"
