...
function matchesVehicle(row: JobOrderRow, vehicleId: string | number) {
...
export default function VehicleJobOrderTable({ vehicleId }: { vehicleId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const mapped = list
          .map(mapItem)
          .filter((item: JobOrderRow) => matchesVehicle(item, vehicleId))
        ...
        setRows([])
        showToast('Failed to load vehicle job orders', 'error')
      } finally {
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Job orders created for the selected vehicle"
