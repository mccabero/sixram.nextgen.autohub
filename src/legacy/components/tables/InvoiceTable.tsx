...
export interface Item { id:number; clientType?:string; invoiceNo?:string; invoiceDate?:string; customerName?:string; joNo?:string; amount?:number; deposit?:number; invoiceType?:string; status?:string }

function mapInvoiceRow(it: any): Item & { createdDate?: string | null } {
  return {
    id: it.id ?? it.Id,
    createdDate: it.createdDate ?? it.CreatedDate ?? it.createdDateTime ?? it.CreatedDateTime ?? null,
    clientType: typeof it.isChangan !== 'undefined'
      ? (it.isChangan ? 'CHANGAN' : 'BOSCH')
      : typeof it.is_changan !== 'undefined'
        ? (it.is_changan ? 'CHANGAN' : 'BOSCH')
        : typeof it.IsChangan !== 'undefined'
          ? (it.IsChangan ? 'CHANGAN' : 'BOSCH')
          : (it.clientType ?? it.client_type ?? it.client ?? ''),
    invoiceNo: it.invoiceNo ?? it.invoice_number ?? it.number ?? '',
    invoiceDate: it.invoiceDate ?? it.date ?? it.invoice_date ?? '',
    customerName: (() => {
    ...
    joNo: it.jobOrder ?? it.joNo ?? it.jo_number ?? it.jobOrderNo ?? '',
    amount: it.totalAmount ?? it.amount ?? it.total ?? 0,
    deposit: it.depositAmount ?? it.deposit ?? 0,
    invoiceType: typeof it.isPackage !== 'undefined'
      ...
      : typeof it.is_package !== 'undefined'
        ...
        : typeof it.IsPackage !== 'undefined'
          ? (it.IsPackage ? 'PACKAGE' : 'REGULAR')
          : (it.invoiceType ?? it.type ?? ''),
    status: (it.jobStatus && (it.jobStatus.name || it.jobStatus.status || it.jobStatus.Name))
      ? (it.jobStatus.name ?? it.jobStatus.status ?? it.jobStatus.Name)
      : (it.status ?? it.statusName ?? it.status_name ?? '')
  }
}

export default function InvoiceTable({ items }: { items?: Item[] }){
  ...
  const { getVoidAction, voidConfirmModal } = useOperationVoidAction<Item>({ operationType: 'invoices', setRows, setLoading })
  ...
  useEffect(()=>{
    ...
    async function load(){
      setLoading(true)
      try{
        const list = await getInvoicesSummary()
        if (!mounted) return
        setRows((Array.isArray(list) ? list : []).map(mapInvoiceRow))
  ...
  const filtered = useMemo(()=>{
    ...
    return rows.filter(r=>{
      ...
      if(!q) return true
      return (
        (r.invoiceNo||'').toString().toLowerCase().includes(q) ||
        (r.customerName||'').toLowerCase().includes(q) ||
        (r.joNo||'').toString().toLowerCase().includes(q)
      )
    })
  ...
  function handleEdit(id:number){ navigate(`/operations/invoice/${id}`) }
  function handleDelete(id:number, status?: string){
  ...
  async function handlePrint(id:number){
    try {
      ...
      showToast(e instanceof Error ? e.message : 'Failed to print invoice report', 'error')
    }
  }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        icon={FileCheck}
        title="Invoices"
        subtitle="Customer invoices and billing records"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          ...
          right={<ListSearchInput value={searchTerm} onChange={(v)=>{ setSearchTerm(v); setPage(0) }} placeholder="Search invoice no, customer, JO…" />}
        />
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState colSpan={showClientType ? 11 : 10} />}
              {paged.map(r=> (
                <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700/60 last:border-b-0 hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4 align-middle text-sm">
                    <ReferenceLinkButton value={r.invoiceNo} onClick={() => handleEdit(r.id)} title="Open invoice" />
                  </td>
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{formatShortDate(r.invoiceDate)}</td>
                  <td className="px-5 py-4 align-middle text-sm font-semibold text-slate-900 dark:text-slate-100">{r.customerName}</td>
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{r.joNo}</td>
                  ...
                  <td className="px-5 py-4 align-middle text-sm text-right text-slate-600 dark:text-slate-300 tabular-nums">{currency(r.deposit)}</td>
                  <td className="px-5 py-4 align-middle"><TypePill t={r.invoiceType} /></td>
                  ...
                  <td className="px-5 py-4 align-middle text-right">
                    <RowActions actions={[
                      ...
                      getVoidAction(r),
                      { kind: 'delete', onClick: ()=>handleDelete(r.id, r.status), label: `delete-${r.id}` },
                    ]} />
                  </td>
                </tr>
              ))}
