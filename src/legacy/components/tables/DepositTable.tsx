...
import { deleteDeposit } from '../../services/operationService'
...
export interface Item { id:number; clientType?:string; referenceNo?:string; customerName?:string; paymentType?:string; transactionDate?:string; amount?:number; status?:string }

export default function DepositTable({ items }: { items?: Item[] }){
  ...
  const { getVoidAction, voidConfirmModal } = useOperationVoidAction<Item>({ operationType: 'deposits', setRows, setLoading })
  ...
  useEffect(()=>{
    ...
    async function load(){
      setLoading(true)
      try{
        const res = await fetch('/api/operations/deposits/summary')
        if (!mounted) return
        ...
        const mapped = (Array.isArray(list) ? list : []).map((it:any) => ({
          ...
          paymentType: it.paymentType ?? it.payment_type ?? it.method ?? '',
          transactionDate: it.transactionDate ?? it.transactionDateTime ?? it.transaction_date ?? it.date ?? '',
          amount: it.depositAmount ?? it.deposit_amount ?? it.amount ?? it.total ?? 0,
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/operations/deposit/add') }
  function handleEdit(id:number){ navigate(`/operations/deposit/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{
      await deleteDeposit(deleteTargetId)
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        icon={PiggyBank}
        title="Deposits"
        subtitle="Track customer deposit transactions and reservations"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          ...
          right={<ListSearchInput value={searchTerm} onChange={(v)=>{ setSearchTerm(v); setPage(0) }} placeholder="Search reference, customer, payment…" />}
        />
              ...
              {paged.map(r=> (
                <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700/60 last:border-b-0 hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4 align-middle text-sm">
                    <ReferenceLinkButton value={r.referenceNo} onClick={() => handleEdit(r.id)} title="Open deposit" />
                  </td>
                  <td className="px-5 py-4 align-middle text-sm font-semibold text-slate-900 dark:text-slate-100">{r.customerName}</td>
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{r.paymentType}</td>
                  ...
                  <td className="px-5 py-4 align-middle text-right">
                    <RowActions actions={[
                      ...
                      { kind: 'print', onClick: ()=>window.open(`/operations/deposit/${r.id}/print`, '_blank'), label: `print-${r.id}` },
                      getVoidAction(r),
