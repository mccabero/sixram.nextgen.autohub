...
import { deleteJobOrder, getJobOrdersByServiceId, openJobOrderFormPdf } from '../../services/operationService'
import {
  ListPageHeader,
  ClientTypeFilter,
  ListSearchInput,
...
export default function ServiceJobOrderTable({ serviceId }: { serviceId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const data = await getJobOrdersByServiceId(serviceId)
        ...
        const matches = list.map(mapItem)
        ...
        setRows([])
        showToast('Failed to load service transactions', 'error')
      } finally {
  ...
  }, [serviceId, logout, navigate, showToast])
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Job orders that used the selected service"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState colSpan={showClientType ? 10 : 9} title="No job orders found" description="This service has not been used in any job orders yet." />}
              {paged.map(row => (
