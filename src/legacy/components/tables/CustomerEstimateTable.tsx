...
import { deleteEstimate, getEstimatesSummary, openEstimateFormPdf } from '../../services/operationService'
import {
  ListPageHeader,
  ClientTypeFilter,
  ListSearchInput,
...
function mapItem(it: any): EstimateRow {
...
function matchesCustomer(row: EstimateRow, customerId: string | number) {
...
export default function CustomerEstimateTable({ customerId }: { customerId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const mapped = list
          .map(mapItem)
          .filter((item: EstimateRow) => matchesCustomer(item, customerId))
        ...
        setRows([])
        showToast('Failed to load customer estimates', 'error')
      } finally {
  ...
  function handleAdd() {
    ...
    navigate(`/operations/estimate/add${customerId ? `?customerId=${customerId}` : ''}`)
  }

  function handleEdit(id: number) {
    navigate(`/operations/estimate/${id}`)
  }

  function handleDelete(row: EstimateRow) {
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Estimates created for the selected customer"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          ...
          right={<ListSearchInput value={searchTerm} onChange={(value) => { setSearchTerm(value); setPage(0) }} placeholder="Search reference, vehicle, plate..." />}
        />
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState colSpan={showClientType ? 10 : 9} title="No estimates found" description="This customer does not have matching estimate records yet." />}
              {paged.map(row => (
                <tr key={row.id} className="border-b border-slate-100 dark:border-slate-700/60 last:border-b-0 hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{formatShortDate(row.estimateDate)}</td>
                  ...
                  <td className="px-5 py-4 align-middle"><TypePill type={row.estimateType} /></td>
