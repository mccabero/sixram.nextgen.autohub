...
import { deleteInspection, getInspectionsSummary, openInspectionFormPdf } from '../../services/operationService'
import {
  ListPageHeader,
  ClientTypeFilter,
  ListSearchInput,
...
function mapItem(it: any): InspectionRow {
  ...
  return {
    ...
    vehicleId: it.vehicleId ?? it.VehicleId ?? it.vehicle?.id ?? it.vehicle?.vehicleId ?? it.vehicle?.vehicle_id,
    clientType: String(clientType || ''),
    referenceNo: String(it.referenceNo ?? it.refNo ?? it.reference ?? ''),
    inspectionDate: String(it.transactionDate ?? it.transactionDateTime ?? it.transaction_date ?? it.date ?? it.inspectionDate ?? it.inspection_date ?? ''),
    customerName,
...
function matchesVehicle(row: InspectionRow, vehicleId: string | number) {
  return String(row.vehicleId ?? '').trim() === String(vehicleId ?? '').trim()
}

export default function VehicleInspectionTable({ vehicleId }: { vehicleId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const data = await getInspectionsSummary()
        if (!mounted) return
        ...
        const mapped = list
          .map(mapItem)
          .filter((item: InspectionRow) => matchesVehicle(item, vehicleId))

        mapped.sort((a, b) => {
          const pa = Date.parse(String(a.inspectionDate || ''))
          const pb = Date.parse(String(b.inspectionDate || ''))
        ...
        setRows([])
        showToast('Failed to load vehicle inspections', 'error')
      } finally {
  ...
  }, [vehicleId, logout, navigate, showToast])
  ...
  function handleAdd() {
    ...
    navigate(`/operations/inspection/add${vehicleId ? `?vehicleId=${vehicleId}` : ''}`)
  }

  function handleEdit(id: number) {
    navigate(`/operations/inspection/${id}`)
  }

  function handleDelete(row: InspectionRow) {
  ...
  async function handlePrint(id: number) {
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Failed to print inspection form', 'error')
    }
  }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Inspections created for the selected vehicle"
            ...
            <tbody>
              ...
              {paged.map(row => (
                <tr key={row.id} className="border-b border-slate-100 dark:border-slate-700/60 last:border-b-0 hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{formatShortDate(row.inspectionDate)}</td>
