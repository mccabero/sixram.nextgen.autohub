...
import { getParameterGroups, deleteParameterGroup } from '../../services/configService'
...
export interface Item { id:number; code?: string; name:string; description?:string }

export default function ParameterGroupsTable({ items = [] }: { items?: Item[] }){
  ...
  useEffect(()=>{
    let mounted = true
    setLoading(true)
    getParameterGroups().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[])

  function isParameterGroupComplete(row: Item) {
    return hasValue(row.code) && hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isParameterGroupComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isParameterGroupComplete), [rows])

  const filtered = useMemo(()=>{
    const q=searchTerm.trim().toLowerCase()
    if(!q) return completenessFiltered
    return completenessFiltered.filter(r=> (r.code||'').toLowerCase().includes(q) || (r.name||'').toLowerCase().includes(q) || (r.description||'').toLowerCase().includes(q))
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/parameter-groups/add') }
  function handleEdit(id:number){ navigate(`/configuration/parameter-groups/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteParameterGroup(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Logical groups for configurable system parameters"
        addLabel="Add Parameter Group"
        onAdd={handleAdd}
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          ...
          right={<ListSearchInput value={searchTerm} onChange={(v)=>{ setSearchTerm(v); setPage(0) }} placeholder="Search code, name..." />}
        />
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={SlidersHorizontal} colSpan={5} />}
              {paged.map(r=> (
