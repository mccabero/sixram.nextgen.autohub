...
import { deleteUnitOfMeasure } from '../../services/configService'
...
export default function UnitOfMeasuresTable({ items = [] }: { items?: Item[] }){
  ...
  function isUnitOfMeasureComplete(row: Item) {
    return hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isUnitOfMeasureComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isUnitOfMeasureComplete), [rows])
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/unit-of-measures/add') }
  function handleEdit(id:number){ navigate(`/configuration/unit-of-measures/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteUnitOfMeasure(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Standardized units for quantities and measurements"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={Ruler} colSpan={4} />}
              {paged.map(r=> (
