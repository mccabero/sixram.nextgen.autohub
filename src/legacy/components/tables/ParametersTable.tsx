...
import { getParameters, deleteParameter } from '../../services/configService'
...
export interface Item { id:number; code?:string; name:string; group?:string }

export default function ParametersTable({ items = [] }: { items?: Item[] }){
  ...
  useEffect(()=>{
    let mounted = true
    setLoading(true)
    getParameters().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[])

  function getGroupName(row: Item) {
    return String((row as any).parameterGroup?.name || row.group || '')
  }

  function isParameterComplete(row: Item) {
    return hasValue(row.code) && hasValue(getGroupName(row)) && hasValue((row as any).description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isParameterComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isParameterComplete), [rows])

  const filtered = useMemo(()=>{
    const q=searchTerm.trim().toLowerCase()
    if(!q) return completenessFiltered
    return completenessFiltered.filter(r=> (r.code||'').toLowerCase().includes(q) || (r.name||'').toLowerCase().includes(q) || getGroupName(r).toLowerCase().includes(q) || ((r as any).description || '').toLowerCase().includes(q))
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/parameters/add') }
  function handleEdit(id:number){ navigate(`/configuration/parameters/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteParameter(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  const getSubtitle = (row: Item) => {
    const description = String((row as any).description ?? '').replace(/\s+/g, ' ').trim()
    if (description) return description
    if (row.code) return `Code ${row.code}`
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        icon={Sliders}
        title="Parameters"
        subtitle="Configurable system parameters and lookup values"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          ...
          right={<ListSearchInput value={searchTerm} onChange={(v)=>{ setSearchTerm(v); setPage(0) }} placeholder="Search code, name, group..." />}
        />
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={Sliders} colSpan={5} />}
              {paged.map(r=> (
                <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700/60 last:border-b-0 hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300 font-mono">{r.code}</td>
                  <td className="px-5 py-4 align-middle">
                    <button onClick={() => handleEdit(r.id)} className="group text-left transition-colors">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-50 text-cyan-700 ring-1 ring-cyan-200 dark:bg-cyan-500/10 dark:text-cyan-300 dark:ring-cyan-500/30">
                          <Sliders size={15} />
                        </span>
                  ...
                  <td className="px-5 py-4 align-middle text-sm text-slate-600 dark:text-slate-300">{getGroupName(r)}</td>
