...
function matchesCustomer(row: JobOrderRow, customerId: string | number) {
...
export default function CustomerJobOrderTable({ customerId }: { customerId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const mapped = list
          .map(mapItem)
          .filter((item: JobOrderRow) => matchesCustomer(item, customerId))
        ...
        setRows([])
        showToast('Failed to load customer job orders', 'error')
      } finally {
  ...
  function handleAdd() {
    ...
    navigate(`/operations/job-order/add${customerId ? `?customerId=${customerId}` : ''}`)
  }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        icon={Wrench}
        title="Customer Job Orders"
        subtitle="Job orders created for the selected customer"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState colSpan={showClientType ? 10 : 9} title="No job orders found" description="This customer does not have matching job order records yet." />}
              {paged.map(row => (
