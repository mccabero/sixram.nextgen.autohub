...
import { getJobStatuses, deleteJobStatus } from '../../services/configService'
...
export default function JobStatusesTable({ items }: { items?: Item[] }){
  ...
  useEffect(()=>{
    let mounted = true
    setLoading(true)
    getJobStatuses().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[])

  function isJobStatusComplete(row: Item) {
    return hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isJobStatusComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isJobStatusComplete), [rows])
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/job-statuses/add') }
  function handleEdit(id:number){ navigate(`/configuration/job-statuses/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteJobStatus(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        icon={Flag}
        title="Job Statuses"
        subtitle="Workflow states for tracking job order progress"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={Flag} colSpan={4} />}
              {paged.map(r=> (
                  ...
                  <td className="px-5 py-4 align-middle">
                    <button onClick={() => handleEdit(r.id)} className="group text-left transition-colors">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-rose-50 text-rose-700 ring-1 ring-rose-200 dark:bg-rose-500/10 dark:text-rose-300 dark:ring-rose-500/30">
                          <Flag size={15} />
                        </span>
