...
function normalizeProduct(row: any): ProductRow {
  return {
    ...
    productGroupName: row?.productGroup?.name ?? row?.productGroup?.Name ?? row?.ProductGroup?.name ?? row?.ProductGroup?.Name ?? '',
    productCategoryName: row?.productCategory?.name ?? row?.productCategory?.Name ?? row?.ProductCategory?.name ?? row?.ProductCategory?.Name ?? '',
    manufacturerName: row?.manufacturer?.name ?? row?.manufacturer?.Name ?? row?.Manufacturer?.name ?? row?.Manufacturer?.Name ?? '',
    supplierName: row?.supplier?.name ?? row?.supplier?.Name ?? row?.Supplier?.name ?? row?.Supplier?.Name ?? '',
  }
}

export default function VehicleModelApplicableProductsTable({ vehicleModelId }: { vehicleModelId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const data = await configService.getVehicleModelApplicableProducts(vehicleModelId)
        ...
        setRows([])
        showToast('Failed to load applicable products', 'error')
      } finally {
  ...
  }, [vehicleModelId, logout, navigate, showToast])
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(row =>
      ...
      || String(row.partNo || '').toLowerCase().includes(query)
      || String(row.productGroupName || '').toLowerCase().includes(query)
      || String(row.productCategoryName || '').toLowerCase().includes(query)
      || String(row.manufacturerName || '').toLowerCase().includes(query)
      || String(row.supplierName || '').toLowerCase().includes(query)
    )
  }, [rows, searchTerm])
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Products linked to this vehicle model"
        ...
        <ListToolbar
          right={
            <ListSearchInput
              ...
              placeholder="Search name, part no, group, category..."
            />
          }
        />
            ...
            <tbody>
              {paged.length === 0 && (
                <EmptyState
                  icon={Boxes}
                  colSpan={9}
                  title="No applicable products found"
                  description="No products are currently linked to this vehicle model."
                />
              )}
              {paged.map(row => (
