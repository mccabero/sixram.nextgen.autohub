...
import { getServiceCategories, deleteServiceCategory } from '../../services/configService'
...
export interface ServiceCategory { id: number; name: string; description?: string }

export default function ServiceCategoriesTable({ items = [], fetchOnMount = true }: { items?: ServiceCategory[]; fetchOnMount?: boolean }){
  const navigate = useNavigate()
  const { showToast } = useToast()
  const { role } = useAuth()
  const [rows, setRows] = useState<ServiceCategory[]>(items)
  ...
  useEffect(()=>{
    ...
    let mounted = true
    setLoading(true)
    getServiceCategories().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[fetchOnMount])

  function isServiceCategoryComplete(row: ServiceCategory) {
    return hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isServiceCategoryComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isServiceCategoryComplete), [rows])
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/service-categories/add') }
  function handleEdit(id:number){ navigate(`/configuration/service-categories/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{
      await deleteServiceCategory(String(deleteTargetId))
  ...
  const getSubtitle = (row: ServiceCategory) => String(row.description ?? '').replace(/\s+/g, ' ').trim()
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Classification of services for organized catalog management"
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={Layers} colSpan={4} />}
              {paged.map(r=> (
                  ...
                  <td className="px-5 py-4 align-middle">
                    <button onClick={() => handleEdit(r.id)} className="group text-left transition-colors">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-violet-50 text-violet-700 ring-1 ring-violet-200 dark:bg-violet-500/10 dark:text-violet-300 dark:ring-violet-500/30">
                          <Layers size={15} />
                        </span>
