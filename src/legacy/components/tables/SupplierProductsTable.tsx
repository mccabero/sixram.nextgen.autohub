...
import { getProductsBySupplierId } from '../../services/managementService'
...
function normalizeProduct(row: any): ProductRow {
  return {
    ...
    manufacturerName: row?.manufacturer?.name ?? row?.manufacturer?.Name ?? row?.manufacturerName ?? '',
  }
}

export default function SupplierProductsTable({ supplierId }: { supplierId?: string | number }) {
  ...
  }, [supplierId, logout, navigate, showToast])
  ...
  return (
    <div className="w-full">
      <ListPageHeader icon={Boxes} title="Supplier Products" subtitle="Products currently linked to the selected supplier" stats={[{ label: 'Total', value: rows.length }]} />
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar right={<ListSearchInput value={searchTerm} onChange={(value) => { setSearchTerm(value); setPage(0) }} placeholder="Search name, part no, group, manufacturer..." />} />
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState icon={Boxes} colSpan={8} title="No products found" description="This supplier does not have any related products yet." />}
              {paged.map(row => (
