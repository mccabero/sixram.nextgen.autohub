...
import { deleteJobOrder, getJobOrdersByProductId, openJobOrderFormPdf } from '../../services/operationService'
import {
  ListPageHeader,
  ClientTypeFilter,
  ListSearchInput,
...
function TypePill({ type }: { type?: string }) {
  ...
  if (normalized === 'PACKAGE') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-500/30">PACKAGE</span>
  if (normalized === 'REGULAR') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-slate-50 text-slate-700 ring-1 ring-slate-200 dark:bg-slate-700/40 dark:text-slate-300 dark:ring-slate-600">REGULAR</span>
  return <span className="text-slate-400 text-xs">-</span>
}

export default function ProductJobOrderTable({ productId }: { productId?: string | number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const data = await getJobOrdersByProductId(productId)
        const list: any[] = Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : Array.isArray(data?.jobOrders) ? data.jobOrders : []
        const mapped = list.map(mapItem).filter((item): item is JobOrderRow => item != null)
        mapped.sort((a, b) => {
        ...
        setRows([])
        showToast('Failed to load product transactions', 'error')
      } finally {
  ...
  }, [productId, logout, navigate, showToast])
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(row => {
      ...
      if (!q) return true
      return String(row.referenceNo || '').toLowerCase().includes(q)
  ...
  function handleDelete(row: JobOrderRow) {
    ...
    if (status !== 'OPEN') { showToast('Only OPEN job orders can be deleted', 'error'); return }
    setDeleteTargetId(row.id)
    setShowDeleteConfirm(true)
  }
  async function handlePrint(id: number) {
    try { await openJobOrderFormPdf(id) } catch (error: any) { showToast(error instanceof Error ? error.message : 'Failed to print job order form', 'error') }
  }
  async function confirmDelete() {
    ...
    try {
      ...
      setIsDeleting(false); setShowDeleteConfirm(false); setDeleteTargetId(null); setSearchTerm(''); setPage(0); setLoading(false)
    }
  }

  return (
    <div className="w-full">
      <ListPageHeader icon={Boxes} title="Product Transactions" subtitle="Job orders that used the selected product" stats={[{ label: 'Total', value: counts.all }]} />
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700 rounded-2xl shadow-card">
        <ListToolbar
          left={showClientType ? <ClientTypeFilter value={selectedClientType} onChange={(value) => { setSelectedClientType(value as any); setPage(0) }} options={[{ key: 'ALL', label: 'All', count: counts.all }, { key: 'BOSCH', label: 'BOSCH', count: counts.bosch, activeClass: 'bg-amber-500 text-white' }, { key: 'CHANGAN', label: 'CHANGAN', count: counts.changan, activeClass: 'bg-sky-500 text-white' }]} /> : null}
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full w-full">
            ...
            <tbody>
              {paged.length === 0 && <EmptyState colSpan={showClientType ? 10 : 9} title="No job orders found" description="This product has not been used in any job orders yet." />}
              {paged.map(row => (
