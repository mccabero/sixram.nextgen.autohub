...
import { getProductCategories, deleteProductCategory } from '../../services/configService'
...
export default function ProductCategoriesTable({ items = [] }: { items?: Item[] }){
  ...
  useEffect(()=>{
    let mounted = true
    setLoading(true)
    getProductCategories().then((data:any)=>{ if(mounted && Array.isArray(data)){ setRows(data) } }).catch(()=>{}).finally(()=> mounted && setLoading(false))
    return ()=>{ mounted = false }
  },[])

  function isProductCategoryComplete(row: Item) {
    return hasValue(row.description)
  }

  const completenessFiltered = useMemo(() => filterByCompleteness(rows, completenessFilter, isProductCategoryComplete), [rows, completenessFilter])
  const completenessFilterOptions = useMemo(() => buildCompletenessFilterOptions(rows, isProductCategoryComplete), [rows])
  ...
  function handleAdd(){ if (!(role === 'ADMIN' || role === 'STAFF')) { showToast('Permission denied', 'error'); return } navigate('/configuration/product-categories/add') }
  function handleEdit(id:number){ navigate(`/configuration/product-categories/${id}`) }
  ...
  async function confirmDelete(){
    ...
    try{ await deleteProductCategory(String(deleteTargetId)); setRows(r=>r.filter(x=>x.id!==deleteTargetId)); showToast('Record deleted','success') }
  ...
  return (
    <div className="w-full">
      <ListPageHeader
        ...
        subtitle="Classification of products for catalog organization"
