...
import {
  BCS_INSPECTION_LAYOUT_KEY,
  BCS_INSPECTION_TEMPLATE_NAME,
  DEFAULT_BCS_INSPECTION_CHECKLIST_GROUPS,
  cloneBcsInspectionChecklistGroups,
  createEmptyBcsRecommendations,
  normalizeBcsInspectionChecklistGroups,
  normalizeBcsRecommendations,
  type BcsCondition,
  type BcsInspectionChecklistGroup,
  type BcsRecommendation,
} from '../utils/bcsInspectionChecklist'
...
const INITIAL_GROUPS: BcsInspectionChecklistGroup[] = cloneBcsInspectionChecklistGroups(DEFAULT_BCS_INSPECTION_CHECKLIST_GROUPS)
...
export default function ManageInspection() {
  ...
  const [groups, setGroups] = useState<BcsInspectionChecklistGroup[]>(() =>
    cloneBcsInspectionChecklistGroups(INITIAL_GROUPS)
  )
  const [recommendations, setRecommendations] = useState<BcsRecommendation[]>(() => createEmptyBcsRecommendations())
  const [templateMeta, setTemplateMeta] = useState<InspectionChecklistTemplateReference | null>({
    name: BCS_INSPECTION_TEMPLATE_NAME,
    revision: 1,
    layoutKey: BCS_INSPECTION_LAYOUT_KEY,
  })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const layoutKey = String(data.layoutKey ?? data.LayoutKey ?? '').trim()
        const nextGroups = layoutKey === BCS_INSPECTION_LAYOUT_KEY
          ? normalizeBcsInspectionChecklistGroups(data.groups ?? data.Groups)
          : cloneBcsInspectionChecklistGroups(INITIAL_GROUPS)
        setGroups(cloneBcsInspectionChecklistGroups(nextGroups))
        setTemplateMeta({
          id: data.id ?? data.Id,
          name: data.name ?? data.Name ?? BCS_INSPECTION_TEMPLATE_NAME,
          revision: Number(data.revision ?? data.Revision ?? 1),
          layoutKey: BCS_INSPECTION_LAYOUT_KEY,
        })
      } catch {
        if (!mounted) return
        setGroups(cloneBcsInspectionChecklistGroups(INITIAL_GROUPS))
        setTemplateMeta({
          name: BCS_INSPECTION_TEMPLATE_NAME,
          revision: 1,
          layoutKey: BCS_INSPECTION_LAYOUT_KEY,
      ...
      try {
        ...
        if (detailsTemplate && typeof detailsTemplate === 'object') {
          setTemplateMeta({
            ...
            name: (detailsTemplate as any).name ?? (detailsTemplate as any).Name ?? BCS_INSPECTION_TEMPLATE_NAME,
        ...
        // RAG items
        const nextGroups = normalizeBcsInspectionChecklistGroups(
        ...
        setRecommendations(normalizeBcsRecommendations(parsedDetails?.recommendations ?? parsedDetails?.Recommendations))
      } catch (err) {
  ...
  function setStatus(groupIdx: number, itemIdx: number, condition: BcsCondition) {
    setGroups(g => g.map((grp, gi) => {
      ...
      return {
        ...
        detailsModelList: grp.detailsModelList.map((it, ii) => {
          ...
          return { ...it, condition: it.condition === condition ? '' : condition }
        })
      }
    }))
  }
  ...
  function setRecommendationField(index: number, field: 'task' | 'item' | 'reason', value: string) {
    setRecommendations(current => current.map((recommendation, currentIndex) =>
      currentIndex === index ? { ...recommendation, [field]: value } : recommendation
    ))
  }

  function addRecommendation() {
    setRecommendations(current => {
      const nextId = current.reduce((maxId, recommendation) => Math.max(maxId, recommendation.id), 0) + 1
      return [...current, { id: nextId, task: '', item: '', reason: '' }]
    })
  }

  function deleteRecommendation(index: number) {
    setRecommendations(current => current.filter((_, currentIndex) => currentIndex !== index))
  }
  ...
  function buildInspectionDetailsPayload() {
    return JSON.stringify({
      ...
      template: templateMeta && (templateMeta.id || templateMeta.name || templateMeta.revision)
        ? { ...templateMeta, layoutKey: BCS_INSPECTION_LAYOUT_KEY }
        : { name: BCS_INSPECTION_TEMPLATE_NAME, revision: 1, layoutKey: BCS_INSPECTION_LAYOUT_KEY },
      groups,
      recommendations,
    ...
    <div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-lg font-semibold">{isAdd ? 'Add BCS Inspection' : 'Manage BCS Inspection'}</h2>
            <div className="mt-1 text-xs text-slate-500">Form version: BCS Checklist v1</div>
          </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
          ...
          <div className="rounded border overflow-hidden">
            ...
            <div className="p-4 flex flex-wrap items-center justify-between gap-3">
              <div className="rounded-lg border border-sky-100 bg-sky-50 px-3 py-2 text-xs text-sky-800">
                This record will keep this form version after saving.
              </div>
              {showIsChanganOption && (
                <div className="flex items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <Printer size={14} /> {printing ? 'Printing...' : 'Print BCS Form'}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded shadow-sm">
          <div className="rounded border overflow-hidden">
            <div className="bg-gray-100 px-4 py-2 flex items-center">
              <div className="text-sm font-medium text-slate-700">BCS Condition Legend</div>
            </div>
            <div className="p-4 flex flex-wrap items-center gap-6 text-sm text-slate-700">
              <div className="flex items-center gap-2">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded border border-emerald-200 bg-emerald-50 text-xs font-semibold text-emerald-700">C</span>
                <span>Good condition</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded border border-amber-200 bg-amber-50 text-xs font-semibold text-amber-700">A</span>
                <span>For adjustment / cleaning</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded border border-rose-200 bg-rose-50 text-xs font-semibold text-rose-700">R</span>
                <span>For replace / bad condition</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex h-7 w-9 items-center justify-center rounded border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-700">N/A</span>
            ...
            <div className="rounded border overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                {grp.detailsModelList.map((it, idx) => (
                  <div key={it.id} className="grid grid-cols-[auto_1fr] gap-3 items-start lg:grid-cols-[auto_1fr_auto]">
                    ...
                    <div className="col-start-2 flex flex-wrap items-center justify-start gap-1 lg:col-start-auto lg:justify-end">
                      {(['C', 'A', 'R', 'NA'] as BcsCondition[]).map(condition => (
                        <button
                          key={condition}
                          onClick={() => setStatus(gi, idx, condition)}
                          disabled={isLocked}
                          className={`h-9 min-w-10 rounded border px-3 text-xs font-semibold transition ${
                          ...
                          aria-label={`${it.name}: ${condition}`}
                          aria-pressed={it.condition === condition}
                        >
        ...
        <div className="bg-white rounded shadow-sm">
          <div className="rounded border overflow-hidden">
            <div className="bg-gray-100 px-4 py-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="text-sm font-medium text-slate-700">List of Recommendation</div>
                <div className="text-xs text-slate-500">Specify recommendations that are not listed above</div>
              </div>
              {!isLocked && (
                <button
                  type="button"
                  onClick={addRecommendation}
                  className="inline-flex items-center justify-center gap-2 rounded border border-sky-700 bg-sky-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:border-sky-800 hover:bg-sky-800 focus:outline-none focus:ring-2 focus:ring-sky-300"
                >
            ...
            <div className="p-4 space-y-3">
              <div className="hidden grid-cols-[1fr_1fr_1.5fr_auto] gap-3 text-xs font-semibold uppercase tracking-wide text-slate-500 md:grid">
                <div>Task</div>
                <div>Item</div>
                <div>Reason</div>
              ...
              {recommendations.map((recommendation, index) => (
                <div key={recommendation.id} className="grid gap-3 md:grid-cols-[1fr_1fr_1.5fr_auto]">
                  <input
                    value={recommendation.task}
                    onChange={event => setRecommendationField(index, 'task', event.target.value)}
                    disabled={isLocked}
                    placeholder="Task"
                    aria-label={`Recommendation ${index + 1} task`}
                    className="rounded border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100"
                  />
                  <input
                    value={recommendation.item}
                    onChange={event => setRecommendationField(index, 'item', event.target.value)}
                    disabled={isLocked}
                    placeholder="Item"
                    aria-label={`Recommendation ${index + 1} item`}
                    className="rounded border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100"
                  />
                  <input
                    value={recommendation.reason}
                    onChange={event => setRecommendationField(index, 'reason', event.target.value)}
                    disabled={isLocked}
                    placeholder="Reason"
                    aria-label={`Recommendation ${index + 1} reason`}
                    className="rounded border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100"
                  />
                  {!isLocked && (
                    <button
                      ...
                      className="inline-flex h-10 w-10 items-center justify-center justify-self-end rounded border border-rose-200 bg-white text-rose-600 shadow-sm transition hover:bg-rose-50 md:justify-self-auto"
                    >
                      <Trash2 size={16} />
                    </button>
                  )}
