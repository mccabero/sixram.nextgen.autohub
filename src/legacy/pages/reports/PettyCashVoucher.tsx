...
import { openPettyCashVoucherReportPdf } from '../../services/operationService'
...
export default function PettyCashVoucher() {
  ...
  useEffect(() => {
    let active = true
    const launchKey = `petty-cash-voucher:${start}:${end}`
    ...
    async function openPdf() {
      if (!start || !end) {
        setError('Start date and end date are required to open the Petty Cash Voucher report.')
        return
      }

      try {
        await openPettyCashVoucherReportPdf(start, end)
        if (!active) return
        navigate('/reports', { replace: true })
      } catch (err: any) {
        if (!active) return
        const message = err?.message || 'Unable to open the Petty Cash Voucher report.'
        setError(message)
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-2xl space-y-4 rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-700">
        <div>
          <h1 className="text-lg font-semibold text-rose-900">Unable to open Petty Cash Voucher</h1>
  ...
  return (
    <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-6 py-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      <div>
        <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Opening Petty Cash Voucher PDF</h1>
