...
export default function ManageParameterGroups(){
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const data: any = await configService.getParameterGroup(id as string)
        if (data) setForm({ code: data.code ?? '', name: data.name ?? '', description: data.description ?? '' })
      }catch(e){ showToast('Error loading Parameter Group', 'error') }
    })()
  ...
  async function handleSave(){
    ...
    try{
      const body: Record<string, any> = { code: form.code, name: form.name, description: form.description }
      ...
      if (isAdd) await configService.createParameterGroup(body)
      else await configService.updateParameterGroup(id as string, body)
      showToast(isAdd ? 'Parameter Group added' : 'Parameter Group updated', 'success')
      navigate('/configuration/parameter-groups')
    }catch(e){ showToast('Error saving Parameter Group', 'error') }
  }

  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{isAdd ? 'Add Parameter Group' : 'Manage Parameter Group'}</h2>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button onClick={()=>navigate('/configuration/parameter-groups')} className="px-4 py-2 border rounded bg-white text-slate-700 hover:bg-slate-50 text-sm">Cancel</button>
