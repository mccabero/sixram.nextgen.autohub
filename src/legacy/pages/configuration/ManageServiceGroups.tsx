...
export default function ManageServiceGroups(){
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const data: any = await configService.getServiceGroup(id as string)
        if (data) setForm({ name: data.name ?? '', description: data.description ?? '' })
      }catch(e){ showToast('Error loading Service Group', 'error') }
    })()
  ...
  async function handleSave(){
    ...
    try{
      ...
      if (isAdd) await configService.createServiceGroup(body)
      else await configService.updateServiceGroup(id as string, body)
      showToast(isAdd ? 'Service Group added' : 'Service Group updated', 'success')
      navigate('/configuration/service-groups')
    }catch(e){ showToast('Error saving Service Group', 'error') }
  }

  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{isAdd ? 'Add Service Group' : 'Manage Service Group'}</h2>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button onClick={()=>navigate('/configuration/service-groups')} className="px-4 py-2 border rounded bg-white text-slate-700 hover:bg-slate-50 text-sm">Cancel</button>
