...
export default function ManageServiceCategories(){
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const data: any = await configService.getServiceCategory(id as string)
        if (data) setForm({ name: data.name ?? '', description: data.description ?? '' })
      }catch(e){ showToast('Error loading Service Category', 'error') }
    })()
  ...
  async function handleSave(){
    ...
    try{
      ...
      if (isAdd) await configService.createServiceCategory(body)
      else await configService.updateServiceCategory(id as string, body)
      showToast(isAdd ? 'Service Category added' : 'Service Category updated', 'success')
      navigate('/configuration/service-categories')
    }catch(e){ showToast('Error saving Service Category', 'error') }
  }

  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{isAdd ? 'Add Service Category' : 'Manage Service Category'}</h2>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button onClick={()=>navigate('/configuration/service-categories')} className="px-4 py-2 border rounded bg-white text-slate-700 hover:bg-slate-50 text-sm">Cancel</button>
