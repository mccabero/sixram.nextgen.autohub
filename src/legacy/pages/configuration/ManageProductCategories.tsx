...
export default function ManageProductCategories(){
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const data: any = await configService.getProductCategory(id as string)
        if (data) setForm({ name: data.name ?? '', description: data.description ?? '' })
      }catch(e){ showToast('Error loading Product Category', 'error') }
    })()
  ...
  async function handleSave(){
    ...
    try{
      ...
      if (isAdd) await configService.createProductCategory(body)
      else await configService.updateProductCategory(id as string, body)
      showToast(isAdd ? 'Product Category added' : 'Product Category updated', 'success')
      navigate('/configuration/product-categories')
    }catch(e){ showToast('Error saving Product Category', 'error') }
  }

  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{isAdd ? 'Add Product Category' : 'Manage Product Category'}</h2>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button onClick={()=>navigate('/configuration/product-categories')} className="px-4 py-2 border rounded bg-white text-slate-700 hover:bg-slate-50 text-sm">Cancel</button>
