...
export default function ManageJobStatuses(){
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const data: any = await configService.getJobStatus(id as string)
        if (data) setForm({ name: data.name ?? '', description: data.description ?? '' })
      }catch(e){ showToast('Error loading Job Status', 'error') }
    })()
  ...
  async function handleSave(){
    ...
    try{
      ...
      if (isAdd) await configService.createJobStatus(body)
      else await configService.updateJobStatus(id as string, body)
      showToast(isAdd ? 'Job Status added' : 'Job Status updated', 'success')
      navigate('/configuration/job-statuses')
    }catch(e){ showToast('Error saving Job Status', 'error') }
  }

  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{isAdd ? 'Add Job Status' : 'Manage Job Status'}</h2>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button onClick={()=>navigate('/configuration/job-statuses')} className="px-4 py-2 border rounded bg-white text-slate-700 hover:bg-slate-50 text-sm">Cancel</button>
